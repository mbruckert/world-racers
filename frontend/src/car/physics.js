  ...
  update(dt, routeCoordinates, offTrackCallback) {
    ...
    if (this.controls.forward) {
      if (!this.prevControls.forward) {
        ...
        this.forwardImpulse = Math.min(1.0, this.forwardImpulse + dt * 1.5);
      }
    } else {
    ...
    if (this.controls.backward) {
      if (!this.prevControls.backward) {
        ...
        this.backwardImpulse = Math.min(1.0, this.backwardImpulse + dt * 1.5);
      }
    } else {
    ...
    const rollingForce = this.rollingResistance * speedMS * Math.sign(speedMS);
    ...
    // Turning
    if (Math.abs(this.carSpeed) > 1e-9 && Math.abs(this.steeringAngle) > 1e-5) {
      const turnRadius =
